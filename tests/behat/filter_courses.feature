...
Feature: Filter the current user's course cards
  ...
  Background:
    ...
    And the following "course enrolments" exist:
      | user     | course      | role    |
      | student1 | Biology 101 | student |
      | student1 | History 101 | student |

  Scenario: Filter courses by name query
    When I log in as "student1"
    And I am on "local/mycoursesfilter/index.php?q=Biology"
  ...
  Scenario: Show a helpful message when no course matches the filter
    When I log in as "student1"
    And I am on "local/mycoursesfilter/index.php?q=Physics"
