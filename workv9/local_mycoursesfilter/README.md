...
- Uses Moodle's core mustache course card partial together with a responsive local wrapper, so the output follows Moodle's course card visual language without forcing the horizontal scrolling carousel used by `core_course/view-cards`.
...
| `filter` | text | One of `all`, `notstarted`, `inprogress`, `completed`, `favourites`, `hidden`. Legacy `status` is still accepted and mapped internally. |
| `status` | text | Legacy alias for `filter`. `any` is treated as `all`. |
